# File: plans.py
# Path: /home/herb/Desktop/ClaudeWatch/Src/ClaudeMonitor/core/plans.py
...
from ClaudeMonitor.Data.EnhancedDatabase import EnhancedDatabaseManager as DatabaseManager
...
class Plans:
    ...
    def get_plan(cls, plan_type: PlanType) -> PlanConfig:
        """Get configuration for a specific PlanType, with learned limits."""
        base_config = cls._build_config(plan_type)
        
        db_manager = DatabaseManager()
        learned_limits = db_manager.get_plan_limit(plan_type.value)
        
        if learned_limits:
            return replace(
                base_config,
                token_limit=learned_limits.get("token_limit", base_config.token_limit),
                message_limit=learned_limits.get("message_limit", base_config.message_limit),
            )
    ...
    def get_plan_by_name(cls, name: str) -> Optional[PlanConfig]:
        """Get PlanConfig by its string name, with learned limits."""
        try:
    ...
    def get_token_limit(
        ...
        if cfg.name == PlanType.CUSTOM.value and blocks:
            from ClaudeMonitor.Core.P90Calculator import P90Calculator
